...
void upload_unicode_char_to_texture(int unicodeChar, int charSize, int bold, float *outCharWidth, float *outCharBaseline, float *outTextureWidth, float *outTextureHeight);
...
static Glyph *find_or_cache_character(unsigned int ch, int charSize, int bold)
{
  for(int i = 0; i < MAX_TEXTURES; ++i) // Naive O(n) lookup for tiny code size
    if (glyphs[i].ch == ch && glyphs[i].charSize == charSize && glyphs[i].bold == bold)
      return glyphs+i;
    else if (!glyphs[i].ch)
    {
      glyphs[i].ch = ch;
      ...
      upload_unicode_char_to_texture(ch, charSize, bold, &glyphs[i].charWidth, &glyphs[i].charBaseline, &glyphs[i].textureWidth, &glyphs[i].textureHeight);
      glyphs[i].bold = bold;
      return glyphs+i;
    }
  return 0; // fail
...
{
  Glyph *glyph = find_or_cache_character(ch, charSize, bold);
  fill_textured_rectangle(x0, y0 - glyph->charBaseline, x0 + glyph->textureWidth, y0 + glyph->textureHeight - glyph->charBaseline, r, g, b, a, glyph->texture);
